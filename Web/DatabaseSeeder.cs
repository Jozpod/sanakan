        ...
        public async Task RunAsync()
        {
            ...
            foreach (var guild in _configuration.Guilds)
            {
                ...
                foreach (var userSeed in guild.Users)
                {
                    ...
                    if (userSeed.Level.HasValue)
                    {
                        user.Level = userSeed.Level.Value;
                        var experience = ExperienceUtils.CalculateExpForLevel(user.Level);
                        user.ExperienceCount = experience;
                    }

                    if (userSeed.ExperiencePercentage.HasValue)
                    {
                        var experience = (ulong)(userSeed.ExperiencePercentage.Value * ExperienceUtils.CalculateExpForLevel(user.Level + 1));
                        user.ExperienceCount += experience;
                    }
