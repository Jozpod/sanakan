...
```console
dotnet publish Web/Web.csproj --configuration Release --runtime win10-x64 --self-contained /p:PublishSingleFile=true /p:DebugType=None /p:DebugSymbols=false --output ./publish
...
| **[`RestartWhenDisconnected - Demonization`](#RestartWhenDisconnected)** | `{Boolean}` | `true` | Restarts the Discord socket client when it is 
disconnected. |
| **[`ReconnectDelay`](#ReconnectDelay)** | `{String\|Timespan}` | `true` | Amount of time to wait before next reconnect attempt. 
