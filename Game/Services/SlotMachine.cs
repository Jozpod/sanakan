    ...
    internal class SlotMachine : ISlotMachine
    {
        ...
        public string Draw(User user)
        {
            ...
            for (var index = 0; index < Rows; index++)
            {
                ...
                {
                    var icon = _row[index, j].Icon(user.SMConfig.PsayMode > 0);
                    rows[index] += icon;
                }
            }
        ...
        public List<List<string>> DrawUCS(User user)
        {
            ...
            for (var i = 0; i < Rows; i++)
            {
                ...
                {
                    tl.Add(_row[i, j].Icon(user.SMConfig.PsayMode > 0));
                }
        ...
        private void Randomize()
        {
            for (int i = 0; i < Rows; i++)
            {
                for (int j = 0; j < Slots; j++)
                {
                    _row[i, j] = Next(0, SlotMachineSlot.max.Value()).ToSMS();
                }
            }
        }
    }
