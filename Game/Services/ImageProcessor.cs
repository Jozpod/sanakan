    ...
    internal class ImageProcessor : IImageProcessor
    {
        ...
        List<LastWatchedRead>? lastWatch = null)
        {
            ...
            if (shindenInfo?.MeanMangaScore != null)
            {
                ...
                image.Mutate(x => x.DrawImage(stats, point, 1));
            }

            point = new Point(5, 71);
            baseImg.Mutate(x => x.DrawImage(image, point, 1));
            using var rwListImage = await GetLastRWList(lastRead, lastWatch);

            point = new Point(330, 69);
            baseImg.Mutate(x => x.DrawImage(rwListImage, point, 1));
        ...
        private static bool AllowStatsOnNoStatsImage(Card card)
        {
            switch (card.Quality)
            {
                case Quality.Zeta:
        ...
        private static Rgba32 GetThetaStatColorString(Card card)
        {
        ...
        private static void CheckProfileImageSize(Image image, Size size, bool stretch)
        {
        ...
        private static string GetStatsString(Card card) => card.Quality switch
        {
        ...
        private static string GetCustomBorderString(Card card)
        {
        ...
        private static async Task<Image?> GetLastRWListCoverAsync(Stream imageStream)
        {
            if (imageStream == null)
            {
                return null;
        ...
        private async Task<Image> GetRWStatsAsync(UserInfo userInfo, string path, bool isManga)
        {
            ...
            {
                var rowArrStr = rowArr[i].ToString();
                baseImg.Mutate(x => x.DrawText(rowArrStr, font, fontColor, new Point(startPointX, startPointY)));
            ...
            IEnumerable<LastWatchedRead>? lastWatch)
        {
            ...
            image.Mutate(x => x.DrawText("Ostatnio obejrzane:", nameFont, fColor, point));

            var items = lastWatch.Take(4).Select(pr => (pr.AnimeCoverUrl, pr.Title, $"{pr.EpisodeNo} / {pr.EpisodesCount}"));
            await DrawSectionAsync(image, fColor, startY, items);

            point = new Point(0, 133);
            image.Mutate(x => x.DrawText("Ostatnio przeczytane:", nameFont, fColor, point));

            items = lastRead.Take(4).Select(pr => (pr.MangaCoverUrl, pr.Title, $"{pr.ChapterNo} / {pr.ChaptersCount}"));
            startY += 128;
            await DrawSectionAsync(image, fColor, startY, items);
        ...
        private async Task DrawSectionAsync(
            Image image,
            Rgba32 fColor,
            int startY,
            IEnumerable<(Uri, string, string)> entries)
        {
            var yOffset = 0;
            var titleFont = GetOrCreateFont(_latoBold, 10);

            foreach (var (url, utitle, text) in entries)
            {
                ...
                var title = utitle.ElipseTrimToLength(29);
                var chapters = text;
        ...
        private async Task ApplyStatsAsync(Image image, Card card, bool applyNegativeStats = false)
        {
            ...
            var point = new Point(startXHp, 190);
            image.Mutate(x => x.DrawText(healthStr, numFont, Colors.Black, point));

            point = new Point(startXAtk, 320);
            image.Mutate(x => x.DrawText(attackStr, numFont, Colors.Black, point));

            point = new Point(startXDef, 440);
            image.Mutate(x => x.DrawText(defenceStr, numFont, Colors.Black, point));
