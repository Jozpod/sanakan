...
{
    /// <summary>
    /// Defines tests for <see cref="DebugModule.GiveawayCardsMultiAsync(ulong, uint, TimeSpan, uint)"/> method.
    /// </summary>
    [TestClass]
    public class GiveawayCardsMultiAsyncTests : Base
    {
        [TestMethod]
        public async Task Should_Give_Away_Cards_And_Send_Confirm_Message()
        {
            var cardCount = 1u;
            ...
            await _module.GiveawayCardsMultiAsync(user.Id, cardCount, duration, repeatCount);
        }
    }
}
