...
namespace DiscordBot.ModulesTests.FunModuleTests
{
    ...
    {
        [TestMethod]
        public async Task Should_Send_Message_Containing_Slot_Machine_Info()
        {
            ...
            await _module.PlayOnSlotMachineAsync("help");
        }

        [TestMethod]
        public async Task Should_Play_Slot_Machine_And_Send_Confirm_Message()
        {
            var utcNow = DateTime.UtcNow;
            var user = new User(1ul, utcNow);
